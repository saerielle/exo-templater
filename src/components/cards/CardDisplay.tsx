...
import { Edit, Gem, Trash2 } from 'lucide-react'

import React, { useMemo } from 'react'

import { abilityToLabel } from '@/lib/utils'

import { Card, EntityImage } from '../../lib/exoloader'

const getSuitColors = (type: string, suit: string) => {
  if (type === 'gear') {
    return {
...
const CardDisplay: React.FC<CardDisplayProps> = ({ card, image, onEdit, onDelete, projectId }) => {
  ...
  const hasGemAbility = useMemo(() => {
    return card.ability1?.id === 'gem' || card.ability2?.id === 'gem' || card.ability3?.id === 'gem'
  }, [card.ability1?.id, card.ability2?.id, card.ability3?.id])

  const colors = getSuitColors(card.type, card.suit)
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      <div className="flex flex-col items-center p-6">
        ...
        >
          ...
          {card.type === 'memory' && !isNil(card.value) && (
            <div className="absolute top-42 left-1/2 transform -translate-x-1/2 z-20">
              <div
                className="w-11 h-11 rounded-full flex items-center justify-center text-lg font-bold shadow-md border-2 border-white"
          ...
          {hasGemAbility && (
            <div className="absolute bottom-3 right-3 z-30">
              <div className="w-6 h-6 flex items-center justify-center text-purple-500 drop-shadow-lg">
