import { Card, CardAbility } from '../../../lib/exoloader'
import { getCardAbilityLabel } from '../../../lib/utils'
import Autocomplete from '../../ui/Autocomplete'
import NumberInput from '../../ui/NumberInput'
import SelectGroup from '../../ui/SelectGroup'
import { cardAbilities, cardAbilitiesById } from './utils'
...
const AbilityItem: React.FC<{
  ...
  const abilityId = (formData[`ability${num}` as keyof Card] as CardAbility)?.id
  const ability =
    cardAbilitiesById[abilityId?.toLowerCase() ?? ''] ??
    (abilityId ? { id: abilityId, name: abilityId } : null)
  const suit = (formData[`ability${num}` as keyof Card] as CardAbility)?.suit
  const value = (formData[`ability${num}` as keyof Card] as CardAbility)?.value
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>Card Abilities:</strong> Each card can have up to 3 abilities that modify
          gameplay. Memory cards typically affect other cards, while gear cards provide passive
          benefits when equipped.
        </p>
      </div>
